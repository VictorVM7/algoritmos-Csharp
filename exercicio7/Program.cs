...
namespace exercicio7
{
    class Program
    {
        ...
        // Calculo da matriz E
        static void MenorE(int[,] matrizA, int[,] matrizB)
        {
            int [ , ] matrizE = new int [4,4];
            for(int l = 0; l < matrizA.GetLength(0); l++){
                for(int c = 0; c < matrizB.GetLength(1); c++){
                    if(matrizA[l,c] < matrizB[l,c]){
                        matrizE[l,c] = matrizA[l,c];
                    }
                    else if(matrizA[l,c] > matrizB[l,c]){
                        matrizE[l,c] = matrizA[l,c];
                    }
                    else{
                        matrizE[l,c] = matrizA[l,c];
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("\nMatriz E (menor entre Matriz A e Matriz B)");
            MenorE(matrizA, matrizB);
        }
    }
}
